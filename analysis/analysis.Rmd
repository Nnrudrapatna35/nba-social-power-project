...
Therefore, linearity, constant variance, and independence are satsified from our new model, but normality still has deviations. However, regression accepts robust departures from normality, so we will proceed with our new model for interpretations. This is displayed here one more time for clarification:
...
The equation of the final model is: `log(TWITTER_FOLLOWER_COUNT_MILLIONS)`-hat = -1.201 + 0.154 * `salary_millionsCent` + -0.003 * `salary_millionsCent * salary_millionsCent` + 2.299 * `w_pctCent` + 5.775 * `w_pctCent * w_pctCent` +  -0.111 * `salary_millionsCent * w_pct_Cent`.
