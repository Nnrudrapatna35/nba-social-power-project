...
Lastly, we consider any potential concerns with multicollinearity:

```{r multicollinearity}
tidy(vif(m_final))
```

As we can see, none of the predictors have VIFs >= 10, so we are not concerned about multicollinearity. 
...
According to our dataset, 5 observations have players who are missing Twitter handles. These values are missing at random because missingness depends on other observed variables (like the person's social media usage, etc.). The probability that a varaible is missing depends on variable information not included in our dataset. We decided to remove the 5 players from our dataset because there were very few observations with missing values, and after remving these variables, we still have 95 observations. The observations with missingness are also random, so the resulting analysis will not be biased because the missingness doesn't differ systematically from the complete observations. In addition, we decided because our analysis is built on determining what predicts the number of Twitter followers, observations without Twitter accounts can't be used to predict for the model.

One other change we've made to our original set was using our final model without LeBron because we decided according to Cook's distance and observation number that LeBron was an outlier in the dataset and had a significant effect on the regression line. We decided to remove LeBron to avoid overestimation in our model. Therefore, we are left with 94 observations, which is still a very large sample and large enough to build a model on that can be generalizable to any population of NBA athletes.

Next, we will check for assumptions for linearity, constant variance, normality, and independence to see if inference on regression coefficients and predictions are reliable.
...
When observing for constant variance, the height of the cloud of points seems to increase as you move from left to right. Points are clustered at the very left then grow to be more spare as you move along the graph. Therfore, constant variance is not satisfied. 

There is no obvious pattern and the shape of the graph seems to be linear. This supports our assumption for linearity. 
...
There looks to be a pattern in the plot because towards the end it curves upward drastically. Therefore, this pattern suggests that interactions or higher-order terms (like quadratic terms) are required. Linearity is not satisfied.
...
nba_social_power_mod2 <- nba_social_power_mod2 %>%
  mutate(predicted = predict.lm(m_final_noLJ), resid = residuals(m_final_noLJ))
```

```{r}
ggplot(data = nba_social_power_mod2, mapping = aes(x = resid)) + geom_histogram() + labs(title = "Distribution of Residuals", x = "Residuals", y = "Count")
```
...
```{r}
ggplot(data = nba_social_power_mod2, mapping = aes(sample = resid)) + stat_qq() + stat_qq_line() + labs(title = "QQ Plot")
```
...
Therefore, because constant variance and normality are not satisfied but linearity and independence are, we will proceed with caution with our model.
...
We will check the variance inflation factor for every predictor variable to see if any are greater than 10.

```{r}
vif(m_final_noLJ)
```

None of the variables are greater than 10, so there are no issues with multicollinearity.
