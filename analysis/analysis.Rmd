...
```{r}
ggplot(data = nba_social_power_mod1, aes(x = SALARY_MILLIONS, y = TWITTER_FOLLOWER_COUNT_MILLIONS,color = W_PCT_CAT)) +
         geom_point() + 
...
Based on the output, the equation of the linear model is: `TWITTER_FOLLOWER_COUNT_MILLIONS`-hat = -2.386 + 0.262 * `ageCent` + 0.017 * `ast_ratioCent` + 0.026 * `off_ratingCent` + 0.086 * `def_ratingCent` + 13.104 * `PIECent` + 7.689 * `reb_pctCent` + 0.052 * `usg_pctCent` + 0.086 * `salary_millionsCent` + 7.095 * `w_pctCent` + 0.063 * `ptsCent` + 3.523 * `ACTIVE_TWITTER_LAST_YEAR` + 0.999 * `salary_millionsCent * w_pct_Cent` + 1.293 * `PIECent * ptsCent`.
...
Hence, the equation of the selected linear model is: `TWITTER_FOLLOWER_COUNT_MILLIONS`-hat = -2.379 + 0.215 * `ageCent` + 28.560 * `PIECent` + 0.115 * `salary_millionsCent` + 6.638 * `w_pctCent` + 3.626 * `ACTIVE_TWITTER_LAST_YEAR` + 1.031 * `salary_millionsCent * w_pct_Cent`.
...
```{r calc-AIC-adjR-include-PIECent}
glance(m1_modV1)$AIC
glance(m1_modV1)$adj.r.squared
```

```{r calc-AIC-adjR-exclude-PIECent}
glance(m1_modV2)$AIC
...
So, the equation of the final model is: `TWITTER_FOLLOWER_COUNT_MILLIONS`-hat = -2.379 + 0.215 * `ageCent` + 28.560 * `PIECent` + 0.115 * `salary_millionsCent` + 6.638 * `w_pctCent` + 3.626 * `ACTIVE_TWITTER_LAST_YEAR` + 1.031 * `salary_millionsCent * w_pct_Cent`.
...
The equation of the final model without LeBron James is: `TWITTER_FOLLOWER_COUNT_MILLIONS`-hat = -1.358 + 0.128 * `ageCent` + 16.105 * `PIECent` + 0.082 * `salary_millionsCent` + 4.081 * `w_pctCent` + 2.490 * `ACTIVE_TWITTER_LAST_YEAR` + 0.472 * `salary_millionsCent * w_pct_Cent`.
