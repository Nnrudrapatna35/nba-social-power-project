...
p1 <- ggplot(nba_social_power_mod, aes(x = SALARY_MILLIONS, y = TWITTER_FOLLOWER_COUNT_MILLIONS)) + 
  geom_point() + 
  labs(title = "Player Salaries vs. Twitter Follower Count",
...
p2 <- ggplot(nba_social_power_mod,
  ...
  geom_point() + 
  labs(title = "Avg. Points Scored vs. Twitter Follower Count",
       x = "Avg. Points Scored",
       y = "Twitter Count, Mil")
...
p3 <- ggplot(nba_social_power_mod, 
  ...
  geom_point() + 
  labs(title = "PIE vs. Twitter Follower Count",
...
p2 <- ggplot(data = m_final_adj_aug, aes(x = w_pctCent, y = .std.resid)) +
  ...
  labs(x = "Percentage of Games Played Won", 
       y = "Std. Resids", 
       title = "Std. Resid vs. Win Percentage")
...
p3 <- ggplot(data = m_final_adj_aug, aes(x = active_twitter_lyear, y = .std.resid)) +
  ...
  labs(x = "Whether The Player Had An Active Twitter Account In 2015-2016", 
       y = "Std. Resids", 
       title = "Std. Resid vs. Active Twitter Last Year")
...
The intercept of the model is -1.292, so a player with an average salary of 11.1 million dollars, an average win percentage of 0.510, and a Twitter account during the 2015-16 season are expected to have a median of exp(-1.292) million, or approximately 274,721, Twitter followers.

The slope estimate associated with the mean centered player salary is 0.142, which means that for every 1 million increase in player salary, a player's median number of Twitter followers are expected to multiply by a factor of exp(0.142) = 1.153, holding all else constant.

The coefficient of the mean centered win percentage is 2.538. This coefficient is misleading, however, as an increase of 1 in win percent is equivalent to 100 %, which is only possible if a team has 0 wins. So, this coefficient is better explained by every 0.1 increase in team win percentage, a player's median number of Twitter followers are expected to multiply by a factor of exp(0.2538) = 1.289, holding all else constant.

The coefficient of the variable that indicates whether players were active on Twitter last year is -2.404, so if an NBA player had a Twitter account during the 2015-16 season, his median number Twitter followers are expected to multiply by a factor of exp(-2.404) = 0.786, holding all else constant.

Our objective in creating the model was prediction of Twitter follower counts, so we did not prioritize ensuring that the interpretations of coefficients made sense. 
...
Wesley Matthews made 17.1 million dollars and had a winning percentage of 39.7% during the 2016-2017 season. Based on our model, his Twitter followers are expected to be exp(-0.726) = 483,850 followers. He actually had 241,000 Twitter followers, so this is actually an overprediction. His salary of 17.1 million dollars may be the reason for this overprediction despite his poor record.
...
To improve our model, we could use a nested F-test to determine whether interactions between mean centered salary and mean centered win percentage, as well as the interactions between mean centered player impact factor and mean centered points scored were significant rather than relying on backwards selection with AIC and adjusted R squared as the selection criteria. One other consideration might be to revisit the full Kaggle dataset and randomly select a different set of explanatory variables, as there were 63 choices. 

In order to make significant progress, we would have to add additional observations. One smaller addition could be to add more players who were not active on Twitter last year. If we had the ability to make larger changes, we could make the model stronger by to building it using on-court statistics from more than just one season, ideally three at minimum. Also, the inclusion of more players in the dataset would allow us to use k-fold cross validation to create a new model after splitting the observations into training and test sets. Lastly, we would like to further test our model on players outside of the dataset by testing its predictive acccuracy on a greater number of randomly selected players.  
...
To stimulate interesting discussion, we could attempt to use social media power/ social media popularity to predict on-court statistics, mirroring Professor Gift's initial analysis. We could also use the data to try to predict other measures of social media popularity, like Instagram followers or Facebook page likes. 
