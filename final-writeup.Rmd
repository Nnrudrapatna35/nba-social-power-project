...
After log-transforming `TWITTER_FOLLOWER_COUNT_MILLIONS`, we discovered thatthe interaction effect `w_pctCent` * `salary_millionsCent` and `ageCent` have p-values that are very large and > 0.05, so we removed them from our model. We will attempt to see if the log transformation of our response variable and removal of insignificant predictors are enough to support our assumptions.
...
ggplot(data = m_final_adj_aug, aes(x = active_twitter_lyear, y = .std.resid)) +
...
There is no distinguishable pattern in any of the plots because there are no discernible curves. The boxplot also shows little change from players with active twitter accounts and those who don't because they both have average standardized residuals around 0. Therefore, we satisfy linearity.
...
ggplot(data = nba_social_power_mod3, mapping = aes(x = std_resid)) + geom_histogram() + labs(title = "Distribution of Standardized Residuals",
                                                                                         ...
                                                                                         y = "Count")
ggplot(data = nba_social_power_mod3, mapping = aes(sample = std_resid)) +
  stat_qq() +
  stat_qq_line() +
  labs(title = "QQ Plot")
```

Normality is not supported because the histogram of residuals is not normal and is now slightly left skewed because the tail of the distribution is longer on the left side of the graph. Points in the QQ plot also don't fall along the diagonal line. Although the histogram is left skewed and the Normal QQ-plot doesn't fall along a diagonal line, most inference methods for regression are robust to some departures from normality, so we will continue with our analysis and assume normality is satisfied.
...
Linearity, constant variance, normality, and independence are all satisfied in this model.
...
`log(TWITTER_FOLLOWER_COUNT_MILLIONS)`-hat = -3.696 + 0.142 * `salary_millionsCent` + 2.538 * `w_pctCent` + 2.404 * `active_twitter_lyear1`.
