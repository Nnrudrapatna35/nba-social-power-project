...
Again, the final model is displayed above. Its equation is: `log(TWITTER_FOLLOWER_COUNT_MILLIONS)`-hat = -3.696 + 0.142 * `salary_millionsCent` + 2.538 * `w_pctCent` + 2.404*`active_twitter_lyear1`

The intercept of the model is -3.696, which means that a player with a salary of 11.1 million dollars (average salary) and win percentage of 0.511 (average win percentage), and no Twitter account during the 2015-16 season are expected to have exp(-3.696) million, or approximately 24,822.619, Twitter followers.
...
The coefficient of `w_pctCent` is 2.538. This coefficient is misleading, as an increase of 1 in win percent is only possible if a team has 0 wins. So, this coefficient is better explained by every 0.1 increase in team win percentage, a player's Twitter followers are expected to multiply by a factor of exp(2.538) = 12.654, holding all else constant.
...
Our model was limited by the fact that the predictor variables only came from one season, and one year of on-court statistics may not be indicative of a player's career statistics. Both salary and winning percentage can have year to year fluctuations due to changes in the NBA salary cap and the prominence of trades, respectively.
