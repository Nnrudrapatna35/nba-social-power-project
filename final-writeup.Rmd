...
The slope estimate associated with the mean-centered player salary is 0.142, which means that for every 1 million increase in player salary, the median number of Twitter followers is expected to multiply by a factor of exp(0.142) = 1.153, holding all else constant.

The coefficient of the mean-centered win percentage is 2.538, which means that for every 1% increase in win percentage, a player's median number of Twitter followers is expected to multiply by a factor of exp(0.2538) = 1.289, holding all else constant.

The coefficient of the variable that indicates whether players were active on Twitter in the 2015-2016 is -2.404, so if an NBA player had a Twitter account during the 2015-16 season, his median number of Twitter followers is expected to multiply by a factor of exp(-2.404) = 0.786, holding all else constant.
...
ggplot(data = nba_social_power_mod, aes(x= AST_RATIO)) + 
  ...
  labs(x = "Assists-to-Turnovers Ratio", y = "Count",
       title = "Distribution of Assists-to-Turnovers Ratio")
